...
void populate_mem_info(struct cgmk_mkey* mr, const char* token, BufferType type, HostMemInfo* out_info) {
    ...
    // 1. Reuse the old code: call export to generate a string containing vhca_id
    // The content of mr_desc string is generated by the old code, and it contains the vhca_id
    size_t desc_len = cgmk_mr_export(mr, (char*)token, strlen(token), mr_desc, sizeof(mr_desc));

    if (desc_len == 0) {
        std::cerr << "Failed to export MR!" << std::endl;
        exit(1);
    }

    // 2. Reuse the old code: deserialize the string to get the original data
    struct desc_data parsed_data = {0};
    deserialize_desc_data(mr_desc, desc_len, &parsed_data); 

    // 3. Fill in our new C++ structure
    out_info->vhca_id = parsed_data.vhca_id;
