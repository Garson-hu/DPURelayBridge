...
cgmk_mr_crossing_reg(struct ibv_pd *pd, char *crossing_mr_desc, size_t crossing_mr_desc_sz)
{
	...
	if (!alias) {
        ...
        if (errno == EREMOTEIO) {
            fprintf(stderr, "Create mkey alias error status 0x%x, syndrome 0x%x\n", DEVX_GET(create_alias_obj_out, out, hdr.status), DEVX_GET(create_alias_obj_out, out, hdr.syndrome));
        }
		return NULL;
	}
